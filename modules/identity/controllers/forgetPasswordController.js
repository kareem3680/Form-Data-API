...
export const sendResetCode = asyncHandler(async (req, res) => {
  await forgetPasswordService.sendResetCode(req.body.email);
  res.status(200).json({
...
export const resendResetCode = asyncHandler(async (req, res) => {
  await forgetPasswordService.resendResetCodeService(req.body.email);
  res.status(200).json({
...
export const resetPassword = asyncHandler(async (req, res) => {
  ...
  const { accessToken, refreshToken } =
    await forgetPasswordService.resetPassword(email, newPassword);
